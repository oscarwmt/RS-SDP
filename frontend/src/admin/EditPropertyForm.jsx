...
const EditPropertyForm = () => {
  const { id } = useParams(); // ID de la propiedad a editar
  ...
  // Cargar los catálogos y los datos de la propiedad al iniciar el componente
  useEffect(() => {
    const fetchCatalogos = async () => {
      try {
        ...
        if (!response.ok) throw new Error("Error al cargar los catálogos");
      ...
      if (id) {
        try {
          ...
          if (data.imagen_destacada_id) {
            ...
            if (!imagenDestacadaResponse.ok)
              throw new Error("Error al cargar la imagen destacada");
